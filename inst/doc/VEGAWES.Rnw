...
\section{Run VEGAWES}
\label{run_VEGAWES}
In order to use this package, all the items mentioned in the installation must be downloaded and Java must be in the current path. The package consists of a \lq\lq parameters.txt \rq\rq file that should be modified by the user before running the package. 
